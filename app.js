...
<body>
  ...
  // استخدام overlay بدل window.open لدعم الهاتف
  const overlay = document.createElement('div');
  overlay.id = 'printOverlay';
  overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,.7);z-index:99999;display:flex;flex-direction:column;align-items:center;justify-content:flex-start;overflow-y:auto;padding:1rem 0';

  const toolbar = document.createElement('div');
  toolbar.style.cssText = 'display:flex;gap:.6rem;margin-bottom:.8rem;flex-wrap:wrap;justify-content:center;';
  toolbar.innerHTML = `
    <button onclick="window.frames['bonFrame'].contentWindow.print()" style="padding:.55rem 1.3rem;background:#6366f1;color:#fff;border:none;border-radius:8px;font-size:1rem;cursor:pointer;font-weight:700">🖨️ طباعة</button>
    <button onclick="document.getElementById('printOverlay').remove()" style="padding:.55rem 1.3rem;background:#ef4444;color:#fff;border:none;border-radius:8px;font-size:1rem;cursor:pointer;font-weight:700">✖ إغلاق</button>`;

  const frame = document.createElement('iframe');
  frame.name = 'bonFrame';
  frame.style.cssText = 'width:82mm;max-width:96vw;border:none;border-radius:8px;background:#fff;box-shadow:0 4px 30px rgba(0,0,0,.4);min-height:300px';
  frame.scrolling = 'no';
  ...
  // ضبط ارتفاع الإطار تلقائياً بعد التحميل
  frame.onload = () => {
    try {
